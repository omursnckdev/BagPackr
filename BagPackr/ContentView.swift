    ...
    private var budgetCard: some View {
        ModernCard {
            VStack(alignment: .leading, spacing: 12) {
                ...
                HStack {
                    // 💰 Input field with fixed currency prefix (just like EditItineraryView)
                    HStack(spacing: 0) {
                        Text(currencySymbol) // now this shows ₺ or $ correctly
...
struct EditItineraryView: View {
    ...
    // Add these for budget text editing
    @State private var budgetText: String = ""
    @State private var isEditingBudget = false
    
    // Define min/max based on locale
    private var minBudget: Double {
        Locale.current.language.languageCode?.identifier == "tr" ? 1000 : 50
    }
